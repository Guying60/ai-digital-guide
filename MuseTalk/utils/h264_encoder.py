...
优化点（相对旧版）：
  1. 优先使用 h264_nvenc（GPU 编码，~0.5ms/帧），失败时自动回落 libx264 ultrafast。
...
def _nvenc_available() -> bool:
    ...
    try:
        ...
        logger.info(f"[h264] h264_nvenc 不可用（{e}），回落 libx264 ultrafast")
        _NVENC_OK = False
    return _NVENC_OK
...
def _build_context(
    ...
    libx264 fallback 选项说明：
      preset=ultrafast  最低 CPU 编码耗时
    ...
    if use_nvenc:
        ...
        ctx.max_b_frames = 0
        ctx.options = {
            "preset":         "ultrafast",   # 比原来的 veryfast 再快 ~30%
            "tune":           "zerolatency",
            "repeat-headers": "1",
            "g":              str(gop),
        }
...
class H264StreamEncoder:
    ...
    def __init__(
        ...
        backend = "h264_nvenc" if use_nvenc else "libx264(ultrafast,fallback)"
        logger.info(f"[h264] 编码器初始化完成：{backend}  {width}x{height}@{fps}fps")
