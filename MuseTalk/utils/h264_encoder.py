...
优化点：
  1. encode_frames_batch：一次性批量 D2H（一个大 DMA），比逐帧 D2H 快 5~10 倍。
  2. encode_frame 保留兼容旧调用方，内部复用批量路径。
"""
...
class H264StreamEncoder:
    ...
    def __init__(
        ...
        self.ctx = _build_context(width, height, fps, bit_rate, gop)
        logger.info(f"[h264] 编码器初始化完成：libx264(ultrafast)  {width}x{height}@{fps}fps")
