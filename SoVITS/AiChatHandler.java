...
public class AiChatHandler extends AbstractWebSocketHandler {
    ...
    @Value("${spring.cosyvoice.ws-url:ws://localhost:7860/ws/tts}")
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) {
        ...
        switch (type) {
            ...
            case "micOn":
                ...
                break;
            case "text":
                String wordText = json.get("text").getAsString();
                log.info("用户输入文本：{}", wordText);
                callAiWithImageAndText(session, wordText);
                break;
            case "photo":
                log.info("有图片");
                String base64Image = json.get("photo").getAsString();
    ...
    public void synthesizeAndStream(String text, WebSocketSession androidSession, WebSocketSession pythonSession) {
        ...
        req.addProperty("type", "synthesize");
        req.addProperty("text", text);
        if (attractionId != null) {
            req.addProperty("attraction_id", attractionId.toString());
        }
            ...
            WebSocketSession cosySession = client.execute(new AbstractWebSocketHandler() {
                ...
                public void afterConnectionEstablished(WebSocketSession cosySession) throws Exception {
                    ...
                    String initJson = String.format(
                            "{\"type\":\"init\",\"attraction_id\":\"%s\",\"session_id\":\"%s\"}",
                            attractionId, sid);
                ...
                protected void handleTextMessage(WebSocketSession cosySession, TextMessage message) throws Exception {
                    JsonObject node = JsonParser.parseString(message.getPayload()).getAsJsonObject();
                    String type = node.has("type") ? node.get("type").getAsString() : "";
                    switch (type) {
                        case "ping":
                            cosySession.sendMessage(new TextMessage("{\"type\":\"pong\"}"));
                        ...
                        case "chunk_end":
                            ...
                            if (pythonSession != null && pythonSession.isOpen()) {
                                ...
                                try {
                                    pythonSession.sendMessage(new TextMessage("{\"type\":\"audio_end\"}"));
                                } catch (IOException e) {
                        ...
                        case "error":
                            String errMsg = node.has("message") ? node.get("message").getAsString() : "unknown";
        ...
        try {
            ...
            WebSocketSession pythonSession = client.execute(new AbstractWebSocketHandler() {
                ...
                public void afterConnectionEstablished(WebSocketSession pythonSession) throws Exception {
                    ...
                    String initJson = String.format("{\"type\":\"init\",\"attraction_id\":\"%s\"}", attractionId);
                ...
                protected void handleTextMessage(WebSocketSession pythonSession, TextMessage message) throws Exception {
                    ...
                    switch (type) {
                        case "ping":
                            ...
                            pythonSession.sendMessage(new TextMessage("{\"type\":\"pong\"}"));
                        ...
                        case "error":
                            String errMsg = node.get("message").getAsString();
                            log.error("[Python WS] 报错：{}", errMsg);
                            break;

                        default:
                            log.warn("[Python WS] 收到未知类型消息: {}", node.get("type").getAsString());
                    }
                }
